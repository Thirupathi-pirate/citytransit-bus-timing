  ...
  container.innerHTML = SECTIONS.map((sec, si) => {
    ...
    const rows = svcs.map((sv, i) => {
      ...
      return `<div class="flex items-center gap-3 px-5 py-2.5 border-b border-outline-variant/20 hover:bg-surface-container-high transition-colors text-sm schedule-row">
        ...
        <span class="font-mono font-bold text-on-surface shrink-0 text-center">${timeDisplay}</span>
        <span class="flex-1 text-xs text-on-surface-variant truncate text-right">${hasVia ? (sv.via || "") : ""}</span>
      </div>`;
    }).join("");
    ...
    return `<div class="bg-surface-container rounded-2xl border border-outline-variant overflow-hidden schedule-section" data-route="${sec.id}" style="animation-delay:${0.08 * si}s">
      ...
      <div class="flex items-center gap-3 px-5 py-2.5 border-b border-outline-variant/30 bg-surface-container/80 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">
        ...
        <span class="shrink-0 text-center">Departure</span>
        <span class="flex-1 text-right">${hasVia ? 'Via' : ''}</span>
      </div>
      ${rows}
    </div>`;
  }).join("");
